...
class SafeQueueQueue(object):
    ...
    def join(self):
        ...
        task_done() to indicate the item was retrieved and all work on it is
        complete.
    ...
    def _qsize(self, len_func=len):
        return len_func(self.queue)
...
class JournaledPersistentQueue(object):
    ...
    RECORD_TYPE_ITEM = b"\x01"
    RECORD_TYPE_JOURNAL = b"\x02"
    ...
    def _open(self, create):
        ...
        if create and exists:
            raise QueueError("refusing to create - file already exists: %s"
                             % (self.filename, ))
    ...
    def _initialize(self):
        """ Initializes ``self._read_pos`` (the location of the record
            currently being read from), ``self._write_pos`` (the location
            where future writes should go) and ``self._item_count``
            (the number of active items in this queue).
            """
        self._file.seek(0)
    ...
    def _read_one(self, start):
        ...
        header = self._file.read(self.HEADER_LEN)
        ...
        record_type = header[0:1]
        if not (record_type == self.RECORD_TYPE_ITEM or
                record_type == self.RECORD_TYPE_JOURNAL):
            return PQRecord(self, start, None, header[0], header[1:],
        ...
        data = data_checksum[:-self.CHECKSUM_LEN]
        checksum = data_checksum[-self.CHECKSUM_LEN:]
        crc32 = zlib.crc32(header + data)
        if crc32 < 0:
            crc32 += (1 << 31)
        actual_checksum = pack_uint(crc32)
        if actual_checksum != checksum:
            return PQRecord(self, start, end, record_type, data,
                            "checksum failed (%r != %r)"
                            % (checksum, actual_checksum))

        return PQRecord(self, start, end, record_type, data, None)
    ...
    def _write_one(self, type, data, active=True):
        ...
        crc32 = zlib.crc32(to_write)
        if crc32 < 0:
            crc32 += (1 << 31)
        to_write += pack_uint(crc32)
...
class PersistentQueue(ThreadsafeQueueBase):
    ...
    def _initialize(self):
        ...
        if len(self._data_files) == 0:
            ...
            self._serial = int(writer_filename[len(self.DATAFILE_PREFIX):],
                               16)
...
def main():
    ...
    tests = [
        ("1-byte", 20000, b"x"),
        ("1k-bytes", 20000, b"x" * 1000),
        ("10k-bytes", 20000, b"x" * 10000),
    ]
    ...
    for (qname, qfactory), (name, num, data) in product(queues, tests):
        ...
        try:
            ...
            operations = [
                ("put/get",
                 lambda: [queue.put_nowait(data), queue.get_nowait()]),
            ...
            for op_name, op in operations:
                ...
                try:
                    ...
                    print(qname, name, num, "%s:" % (op_name, ),
                          duration, stats)
            queue.close()
        finally:
